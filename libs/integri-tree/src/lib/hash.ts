...
export function hashRoot(root: P.Proof): string {
  if (root && 'NodeHash' in root) {
    // If the root is a NodeHash, return its hash
    return root.NodeHash.hash
  } else if (root && 'HashNode' in root) {
    // If the root is a HashNode, combine three hashes
    const { hash, left, right } = root.HashNode
    return combineThreeHashes(hash, hashRoot(left), hashRoot(right))
  } else {
