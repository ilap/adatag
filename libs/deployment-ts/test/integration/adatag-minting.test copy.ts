    ...
    test(`Mint (${String(i++).padStart(3, ' ')}): should ${isValid ? '\x1b[32mPASS\x1b[0m' : '\x1b[31mFAIL\x1b[0m'}: "${adatag}"`, async () => {
      ...
      const validFrom =
        translucent.provider instanceof Emulator
          ...
          : Date.now()
      const validTo = Math.ceil(validFrom * 1.002)
      ...
      //const timelockActive = bd.adatagMinting.params.deactivationTime.epoch > now

      console.warn(`$$$$$$$$$$$$$$$$$$$$ TL ACTIVE: ${validFrom} ${validTo} ${bd.adatagMinting.params.lockingDays.ms}`)
      // ${bd.adatagMinting.params.deactivationTime.epoch} ${timelockActive}`)


      if (useTimelock) {
        // Times are in milliseconds in Plutus Core
        const deadLine = BigInt(
          validTo + bd.adatagMinting.params.lockingDays.ms
        )
        ...
        const deposit = calculateDeposit(adatag, 1750, 15, 6)
      ...
      ///console.warn(`##### TX2 ${stringifyData(tx2)}`)
      const ttx = useTimelock ? tx2 : tx1; // If useTimelock is false, simply assign tx1 to ttx
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            console.warn(`@@@@@@@@@@@ 3. ERROR: ${eee}`)
            expect(isValid).toBe(false)
          }
          //console.log(`####ERROR? ${txHash}`)
        } catch (ee) {
          console.warn(`@@@@@@@@@@@ 2. ERROR: ${ee}`)
          expect(isValid).toBe(false)
        }
      } catch (e) {
