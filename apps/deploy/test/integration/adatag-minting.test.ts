...
import { Data, Emulator, Translucent, fromText, toText, AddressDetails } from 'translucent-cardano'
...
describe(`Adatag minting (${elemsNumber})`, async () => {
  ...
  for (const adatag of adatags) {
    ...
    test(`Mint (${String(i++).padStart(3, ' ')}): should ${
      ...
      const oldState = Data.from(authUtxo.datum!, P.StateHolderStateHolder.oldState)
      ...
      const timeStamp = translucent.provider instanceof Emulator ? translucent.provider.now() : Date.now()

      // zeros the millisecond parts of the validity range
      const from = Math.floor(timeStamp / 1000) * 1000
      const to = from + 60000

      const tx1 = translucent.newTx().collectFrom([authUtxo], Data.void())
      ...
      if (useTimelock && timelockActive) {
        ...
        tx1.payToContract(bd.timelockScript.scriptAddress, { inline: datumCbor }, { lovelace: deposit })
      }
      ...
      const tx = tx1
        ...
        .payToContract(bd.stateholderScript.scriptAddress, { inline: state }, authToken)
