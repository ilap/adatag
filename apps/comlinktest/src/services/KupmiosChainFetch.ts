...
import { ConfigLoader, apiUrls } from '@adatag/shared/config'

import { ChainData, DataStoreService, ChainFetchService, SyncState} from './types.ts'
...
export class KupmiosChainFetch implements ChainFetchService {
  ...
  async fetchTip(): Promise<number> {
    const response = await fetch(`${apiUrls.Kupo.Custom}/checkpoints`)
  ...
  async fetchDataChunks(): Promise<void> {
    try {
      ...
      const url = `${apiUrls.Kupo.Custom}/matches/${policyId}.*?${queryParams}`
      ...
      parser.onValue = async chunk => {
        ...
        this.onStatusChange?.({ state: 'syncing', message: `Syncing. fetched adatag ${adatag}, from slot ${from++} of ${to}`})
        await this.dataStore.updateDatabase(chunkOfData)
      }

      // Store the last processed slot at the end of processing as a checkpoint.
      parser.onEnd = async () => {
        this.onStatusChange?.({ state: 'syncing', message: `Last slot is ${to}`})
        await this.dataStore.setLastSlot(to)
      }
      // Parse response stream
      await this.parseResponseStream(response, parser)
      this.onStatusChange?.({ state: 'synced', message: `Synced is done`})
    } catch (error) {
  ...
  async fetchStateDatum(authNft: string): Promise<ChainState> {
    try {
      ...
      const url = `${apiUrls.Kupo.Custom}/matches/${authPolicyId}.${code}?unspent`
      const response = await fetch(url)
      ...
      // Get the datum...
      const datRes = await fetch(`${apiUrls.Kupo.Custom}/datums/${eutxo.datum_hash}`)
