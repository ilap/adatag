...
const collectorAddress: string = await handleIO(dir, collector + ".addr", () => translucent.selectWalletFromSeed(collectorSeed).wallet.address());
//const collectorPkh: string = getAddressDetails(collectorAddress).paymentCredential?.hash || "";

const ownerSeed: string = await handleIO(dir, owner + ".seed", translucent.utils.generateSeedPhrase);
const ownerAddress: string = await handleIO(dir, owner + ".addr", () => translucent.selectWalletFromSeed(ownerSeed).wallet.address());

const userSeed: string = await handleIO(dir, user + ".seed", translucent.utils.generateSeedPhrase);
const userAddress: string = await handleIO(dir, user + ".addr", () => translucent.selectWalletFromSeed(userSeed).wallet.address());
