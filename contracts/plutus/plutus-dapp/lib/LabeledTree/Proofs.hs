...
data ProofTree
  = ProofLeaf Hash
  | ProofNode Hash ProofTree ProofTree
...
rootHash' tree = case tree of
  ProofLeaf h -> h
  ProofNode vh l r -> combineThreeHashes vh (rootHash' l) (rootHash' r)
...
checkAppend e tsize rootHash updatedRootHash nuv nav proof =
  ...
  checkProof False rootHash && checkProof True updatedRootHash
  where
    nuHash = hashVal nuv
    naHash = hashVal nav
    ...
    checkProof isUpdated root = hasNu && hasNa && root' == root
      where
        -- go :: Bool -> Bool -> ProofTree
        go hasNu' hasNa' (ProofLeaf h) = (hasNu', hasNa', h)
        go hasNu' hasNa' (ProofNode vh l r)
          | isUpdated && isAny = (lnu || rnu, lna || rna, combineThreeHashes nvh nlh nrh)
          | otherwise = (lnu || rnu, lna || rna, combineThreeHashes vh lh rh)
          where
            enu = vh == nuHash
            ena = vh == naHash
            hasNu'' = hasNu' || ena
            ...
            (lnu, lna, lh) = trace "l" $ go hasNu'' hasNa'' l
            (rnu, rna, rh) = trace "r" $ go hasNu'' hasNa'' r

            isAny = enu || ena
            leafh = combineThreeHashes (hashVal leaf) eh eh
            nvh = if enu then hashVal nuv' else hashVal nav'
            (nlh, nrh) = if lh == eh then (leafh, rh) else (lh, leafh)
